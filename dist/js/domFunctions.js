...
export const displayApiError = (statusCode) => {
    const propMsg = toProperCase(statusCode.message)
    updateWeatherLocationHeader(propMsg)
    updateScreenReaderConfirmation(`${propMsg}. Please try again.`)
}
...
export const updateDisplay = (weatherJson, locationObj) => {
    fadeDisplay()
    clearDisplay()
    // check the api json data to understand what kind of data it returns (for example 02d)
...
const getWeatherClass = (icon) => {
    const firsTwoChars = icon.slice(0, 2)
    ...
    let weatherClass
    if (weatherLookup[firsTwoChars]) {
        weatherClass = weatherLookup[firsTwoChars]
    } else if (lastChar === "d") {
        weatherClass = "clouds"
    } else {
        weatherClass = "night"
...
const buildScreenReaderWeather = (weatherJson, locationObj) => {
    ...
    ${Math.round(Number(weatherJson.current.temp))}°${tempUnit} in ${location}`
}
...
const createMainImgDiv = (icon, altText) => {
    ...
    iconDiv.id = "icon"
    const faIcon = transalteIconToFontAwesome(icon)
...
const transalteIconToFontAwesome = (icon) => {
